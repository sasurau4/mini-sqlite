...
#define INVALID_PAGE_NUM __UINT32_MAX__
...
const u_int32_t LEAF_NODE_NEXT_LEAF_OFFSET = LEAF_NODE_NUM_CELLS_OFFSET + LEAF_NODE_NUM_CELLS_SIZE;
...
const u_int32_t LEAF_NODE_LEFT_SPLIT_COUNT =
...
const u_int32_t INTERNAL_NODE_CELL_SIZE =
    INTERNAL_NODE_CHILD_SIZE + INTERNAL_NODE_KEY_SIZE;
/* Keep this small for testing */
const u_int32_t INTERNAL_NODE_MAX_CELLS = 3;
...
u_int32_t *internal_node_child(void *node, u_int32_t child_num)
{
  ...
  {
    u_int32_t *right_child = internal_node_right_child(node);
  ...
  {
    u_int32_t *child = internal_node_cell(node, child_num);
...
u_int32_t get_node_max_key(Pager *pager, void *node)
{
...
void create_new_root(Table *table, u_int32_t right_child_page_num)
{
  ...
  *internal_node_child(root, 0) = left_child_page_num;
  u_int32_t left_child_max_key = get_node_max_key(table->pager, left_child);
...
{
  /**
   * Return the index of the child witch should contain
   * the given key.
   */
  ...
  /* Binary search */
  u_int32_t min_index = 0;
  u_int32_t max_index = num_keys; /* There is one more child than key */
...
/* Define for suppressing warning */
...
void internal_node_insert(Table *table, u_int32_t parent_page_num, u_int32_t child_page_num)
{
  ...
  u_int32_t child_max_key = get_node_max_key(table->pager, child);
...
void internal_node_split_and_insert(Table *table, u_int32_t parent_page_num, u_int32_t child_page_num)
{
  ...
  u_int32_t old_max = get_node_max_key(table->pager, old_node);

  void *child = get_page(table->pager, child_page_num);
  u_int32_t child_max = get_node_max_key(table->pager, child);
  ...
  u_int32_t *old_num_keys = internal_node_num_keys(old_node);

  u_int32_t cur_page_num = *internal_node_right_child(old_node);
  ...
  *internal_node_right_child(old_node) = *internal_node_child(old_node, *old_num_keys - 1);
   ...
   */
  u_int32_t max_after_split = get_node_max_key(table->pager, old_node);

  u_int32_t destination_page_num = child_max < max_after_split ? old_page_num : new_page_num;
...
void leaf_node_split_and_insert(Cursor *cursor, u_int32_t key, Row *value)
{
  ...
  u_int32_t old_max = get_node_max_key(cursor->table->pager, old_node);
  ...
  *(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
  ...
  else
  {
    ...
    u_int32_t new_max = get_node_max_key(cursor->table->pager, old_node);
...
Table *db_open(const char *filename)
{
  ...
  {
    // New database file. Initialize page 0 as lead node.
